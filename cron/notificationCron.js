...
cron.schedule("* * * * *", async () => {
  ...
  try {
    ...
    for (const budget of budgets) {
      ...
      const lastMsg = recentNotif?.message || "";
      const already80 = lastMsg.includes("80%");
      const already100 = lastMsg.includes("exceeded");
    ...
    for (const goal of goals) {
      ...
      const lastMsg = recentGoalNotif?.message || "";

      const reached50 = percent >= 50 && percent < 80 && !lastMsg.includes("50%");
      const reached80 = percent >= 80 && percent < 100 && !lastMsg.includes("80%");
      const reached100 = percent >= 100 && !lastMsg.includes("Achieved");
